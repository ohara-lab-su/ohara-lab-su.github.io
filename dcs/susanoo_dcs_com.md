...
サーバー・クライアント通信と、
サーバー・サーバー間通信を分けて理解するよりもこれらを総括して「ノード間通信」としてまとめて、
ノード間通信のモデルとして理解した方が理解しやすい。
...
自然に身につくかもしれない。
ZMQ は軽量コンパクトで高速な通信用ライブラリであり、
socket 通信の上にメッセージキューやルーティング機能を
追加した軽量メッセージミドルウェアである。
現代の通信の最初の入り口でもあり、
それなりに最後まで付き合うライブラリでもある。
これにより通信パターンを理解することは割と行われている。

ソケットの通信パターンは大きく、REQ/REP、ROUTER/DEALER、PUB/SUB、
に分かれる。 
Web 通信は REQ/REP、ZMQ を使っていると ROUTER / DEALER 
...
HTTP は request / response 型の通信であるが、それ自体が処理系の同期・非同期を決めるわけではない。
クライアントが応答を待つ同期的な使い方も、async I/O などを使った非同期・並行処理も可能である。
ここでは通信パターンと、実装上の同期・非同期は分けて考える。

また、たとえば、放射光設備で言えば、MADOCA2 は ZMQ を通信基盤としている。
TANGO は現在も command / attribute の request / reply に CORBA、event 配信に ZMQ を用いる構成である。
なお TANGO のクライアント API 自体には、同期・非同期の両方の request model が用意されている。
...
通信モデルとして抽象的に見れば、
1つのクライアントが複数のデバイスサーバに
直接アクセスする構造になっているため、
REQ/REP 型通信の集合として理解することもできる。
...
BL774型の味噌は開発・保守が高コストなプロトコル開発やめる。
BLに対してオーバースペック気味の本格的な分散プロトコルをやめる。
同時に運用開発コストも web 技術中心にすることで一気に下げる。
ことを軸に運用・開発体制から見直したことにある。
(私見です)

以下は時代に一度逆行するかもしれないが、
現状を正しく把握するために
プロトコルそれ自体に分散向けのプロトコルを使った場合をまとめていきたい。
...
ひっくるめてデバイスサーバーと呼ぶが。
MADOCA ではデバイスサーバーと呼ぶ文化はない。
MADOCA の文脈ではサーバーとは MS を指す言葉で、
EM はサーバーとは呼ばないからだ。
ただ、TANGO 世界を使い倒したあとだと面倒なので、
EM もデバイスサーバー呼ぶようになってしまう。
初学的な意味では普通にサーバープロセスっぽいし。
MADOCA の後発版では、TANGO に寄せて、BL での運用上の
利便のために、EM に MS を必ずセットする構成にして
展開していた。
これも良くも悪くも反対勢力が多く
MS をおくことでノードの粒度をさらに細かくして、EM の役割と
MS の役割の配置の柔軟性が、MADOCA らしいネットワーク・機器制御
である。だから、MADOCA に詳しい人ほど反対するという、
めんどくさい思想・運用的な問題が多くあった。
...
#### ブローカー型 (MQTT)
中央に broker を置いて topic ベースでメッセージを配信する。
管理が容易だが broker が単一障害点になる。
つまり中央が落ちると全滅。遅延が増える
