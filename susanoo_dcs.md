...
前提であるために、統一的にデバイスサーバーでデバイスサーバーを管理するという発想が薄かった。マネージメントの通信量が
尋常ではなくなり、巨大な実験システムを一元的にネットワーク分散管理するシステムでは、デバイスサーバーでデバイスサーバーを
起動するという考えは合わないという結論だった。 その代わりに windows/linux のプロセス管理の仕組みを積極的に使っていく発想になる。
