...
存在しなかった。コントロールできても、それは問い合わせた時だけ機能する仕組みだ。そうでないと、マネージメントの通信量が
尋常ではなくなる。巨大な実験システムを一元的にネットワーク分散管理するシステムが前提の元だといろいろと制限が多くなる。
...
# 現在の世界における、分散プロトコル(with web系プロトコル) のトレンド
