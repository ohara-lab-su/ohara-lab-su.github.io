...
スサノオでは、分散環境を構築するにあたって、
単純に「より高機能な分散プロトコルを採用すればよい」とは考えていない。

スサノオでは [MADOCA/DARUMA](https://user.spring8.or.jp/sp8info/?p=37181) や
[TANGO](https://www.tango-controls.org/)

放射光施設では、
MADOCA/DARUMA のようなノード間のメッセージルーティングまで含めた
本格的な分散制御システム、TANGO のようなデバイスサーバーを中心とした分散制御、
そして BL774 のような Web 技術を基盤とした比較的シンプルな構成など、
異なる思想の制御システムが実際に利用されている。

これらにはそれぞれ利点があり、どれか一つが常に優れているわけではない。
本格的な分散プロトコルを用いれば、ノード間通信、非同期通信、メッセージルーティング、
イベント配信などをシステム基盤として扱うことができる。
一方で、その機能を支えるプロトコル、Framework、運用環境の開発・保守も必要になる。

結局のところ、

**プロトコルを高度にするのか、実装を高度にするのか、運用を高度にするのか。
あるいは、用途を限定することでシステム全体をシンプルに保つのか。**

という選択になる。

スサノオは、放射光におけるこれらの制御システムの利用・開発経験を踏まえた上で、
研究室から SPring-8 の 1 BL 程度までの実験環境を主な対象とし、
意図的に最後の選択を取っている。

つまり、MADOCA/DARUMA や TANGO のような本格的な分散制御基盤を
小さく作り直すことは目的としない。
個々のデバイスサーバーをできるだけ独立させ、
一般的な通信技術を利用して必要なものだけを組み合わせることで、
開発・運用・保守を小さく保つことを優先する。

この判断の背景となる分散制御モデルとノード間通信については、
以下にまとめている。

1. [機器制御における分散モデル](susanoo_dcs_intro.html)  
   中央集権型、デバイスサーバー型、本格的な分散制御、
   BL774 / TANGO / MADOCA などの構成の違い
...
スサノオの基本モデルは、BL774 と同様に、
それぞれのデバイスサーバーへクライアントから直接接続する
シンプルな構成である。

これは、本格的な分散通信を実装できないためではなく、
対象とする規模と用途に対して、
**通信基盤そのものの複雑性をできるだけ持ち込まない**
という設計上の選択である。

BL774 が主に採用している REST API は、高速な通信方式ではない。
一方で、HTTP / Web を基盤とする技術は非常に広く普及しており、
開発環境、ライブラリ、デバッグ手段も豊富である。
モーター移動、測定条件設定、状態取得など、
通信速度が支配的でない機器制御では、この簡便性は大きな利点となる。

そのためスサノオでは REST / FastAPI を基本的な選択肢としながら、
画像転送や高頻度なデータ交換など、より高速な通信が必要な場合には
gRPC を利用できるようにしている。
つまり、すべての機器を一つの通信方式へ押し込むのではなく、
同じ DeviceClass に対して用途に応じた通信 Frame を選択する。

さらに高度な分散通信が必要な領域では、
DDS や ROS2 のような PUB/SUB を中心とした構成も有力である。
特にロボットとの連携では DDS / ROS2 との接続は無視できない。

ただし、スサノオそのものを DDS ベースの巨大な分散基盤にするのではなく、
必要な領域で接続可能にする方針を取る。

**REST、gRPC、TANGO、DDS のどれか一つを正解とするのではなく、
機器制御クラスを長く利用できる資産として残し、
通信方式は用途に応じて選択・交換できるようにする。**
