...
3. サーバー側の処理されてしまう処理を栗餡と側にしたいなど(たとえばファイル保存)の例外処理を書く
4. スサノオフレームにより自動で作られるデバイスクライアント用いて、 実験制御プログラムを書く。

実質的には最初の 1のステップだけで、デバイスサーバー公開までのステップはほぼ終わる

3の作業をより簡略化するために、1 をうまく作ると良い。すでに我々他用意しているものはこの後のセクションにある。
これらの具体的作業を下記に記す
...
1. スサノオに関係なくデバイス制御クラス(プログラム)を書く。
2. スサノオフレームを用いて、デバイスサーバーの**API定義**を記述する (ある意味これがサーバを作る作業の本丸だが、実はただのコピペ作業)
