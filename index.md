...
BL774互換としての立ち位置は、RestAPI (774 reset frame のコード) や gRPC 通信を軸としているところに由来している。
同じく、get や post のルールをBL774側と真似ているところもある。
ただ、マネージメントやAPIセットにははまだ大きな規定はない。つまり774APIセットに対応しているわけではない。
今の所スサノオは透過型なので制御クラス側にAPIの規定は任せている。
