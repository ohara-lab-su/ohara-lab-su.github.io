...
以下は各機器ごとに**制御プログラム**と
スサノオに対応した**デバイスサーバー**が公開されている。そのドキュメントを以下に示す。
...
Universal Robots では、
コントローラ内部の URControl がロボットのモーション制御とプログラム実行を担い、
その上でロボット言語である URScript が実行される。

TP 上の PolyScope はロボットの操作・プログラミング環境であり、
そこで作成したプログラムも最終的には URScript としてコントローラで実行される
。一方、PC からは PolyScope を介さず URScript をコントローラへ直接送信することもできる。

さらに外部制御用として、ロボット状態や I/O などを周期的に交換する RTDE などの通信 I/F が用意されている。
デンソーウェーブの ORiN2 のような共通デバイスフレームワークを上位に置く構成ではなく、
URControl を中心として URScript や RTDE などの I/F を直接公開する構成となっている。
