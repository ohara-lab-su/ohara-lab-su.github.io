...
[スサノオ](#susanoo)は **DeviceClass ベース**のシンプルな構成を基本としている。
基本単位は、特定の Framework や通信 protocol に依存しない、
個別の **機器制御 class** である。
各機器制御 class は[スサノオ](#susanoo)とは独立して単独で利用できる。

ネットワーク越しの利用が必要な場合にのみ、
REST (**SPring8-BL774互換風味**) / gRPC などの薄い通信 Frame を付加してデバイスサーバーとして利用する。
以下では、各機器ごとの **制御プログラム (DeviceClass)** と、
[スサノオ](#susanoo)に対応した **デバイスサーバー**を示す。

[スサノオ](#susanoo)を利用しない場合でも、下記の制御 class はそのまま利用できる。
...
放射光施設で実際に運用されてきた複数の制御システムでは、
分散通信、デバイスサーバー、マネージメント、運用の責務をどこに持たせるかがそれぞれ異なる。
スサノオでは、それらの構成と運用を踏まえた上で、
研究室から SPring-8 の 1 BL 程度までの実験環境に必要な範囲を選択している。

SPring-8 の [MADOCA/DARUMA](https://user.spring8.or.jp/sp8info/?p=37181) では、
メッセージサーバーと機器制御を分離し、ノードをまたいだメッセージルーティングを含む
分散制御を共通基盤として扱うことができる。
ノード間通信そのものを制御基盤の機能として持つため、
複数のネットワークや多数の機器を横断する構成まで一つの体系で扱える。

ESRF を起源とする [TANGO](https://www.tango-controls.org/) では、
Device / Device Class / Device Server を中心に、
デバイスを分散オブジェクトとして扱う構成が取られている。
デバイスサーバーの起動や名前解決、データベースなどのマネージメントも含め、
制御システムとして一貫した仕組みを持つ。
スサノオで用いている Device、DeviceClass、Device Server という名称や考え方も、
TANGO の影響を強く受けている。

一方、SPring-8 の [BL774](https://user.spring8.or.jp/sp8info/?p=42759) では、
本格的な分散ルーティングを共通プロトコル側に持たせず、
個々のデバイスサーバーへクライアントから直接アクセスする構成を基本としている。
...
共通基盤として利用できる。
その反面、その共通基盤自体を継続して導入・運用・保守する必要がある。

逆に、個々のデバイスサーバーをできるだけ独立させ、
一般的な通信技術で直接接続する構成では、
...
その規模では、MADOCA/DARUMA のようなノード間ルーティングをシステム全体の必須機能とするよりも、
...
通信、上位の実験シーケンス、解析、自動化などは必要に応じてその外側で組み合わせる。
共通基盤が担う責務を対象規模に応じて限定することで、
個々の機器制御資産を特定の Framework や通信方式から切り離して維持する。

ここで重要なのは、通信方式を基本単位にしないことである。
スサノオの基本単位は、特定の通信 protocol や Framework に依存しない DeviceClass である。
DeviceClass は単独でも利用でき、ネットワーク越しに利用するときだけ通信 Frame を付加する。
REST / FastAPI、gRPC、TANGO、将来の DDS などは、
DeviceClass の外側に置かれる交換可能な通信層として扱う。
...
スサノオの基本モデルは、
[BL774](https://user.spring8.or.jp/sp8info/?p=42759) と同様に、
必要なデバイスサーバーへクライアントから直接接続する
サーバー・クライアント型の構成である。

通信モデルとして見ると、個々のデバイスとのやり取りは基本的に request / reply で完結する。
複数のデバイスを使う場合にも、中央のメッセージルータを必須とせず、
クライアントが必要なデバイスサーバーへそれぞれ接続する。
デバイスサーバー同士を一つの分散プロトコルで結びつけることを、
システム全体の前提にはしない。

この構成では、各デバイスサーバーがほぼ独立する。
一方、クライアント側では透過型プロキシを使うため、
利用者はネットワーク越しのデバイスであっても、
ローカルな Python オブジェクトに近い形で扱うことができる。
つまり、通信基盤を小さくしても、実験プログラム側まで低水準な通信処理を露出させる必要はない。

BL774 がメインとして採用している REST API は高速通信を目的とした方式ではない。
しかし、Web に立脚した技術体系の普及率と裾の広さ、簡便さは捨てがたい。
ライブラリ、開発環境、デバッグ手段が広く利用でき、
特定の制御 Framework の知識を持たない開発者でも扱いやすい。

モーター移動、測定条件設定、状態取得など、
通信時間が装置動作全体の支配要因にならない機器制御では、
REST API の速度よりも開発効率、可読性、保守性、一般的な技術であることの利点が大きい。
BL774 のサブセットとしての立ち位置を持つスサノオも、この考え方を基本としている。

<img src="fig/ese_774.005.png" width="70%" style="display:block; margin:auto;">

一方で、画像転送、高頻度のデータ交換、イベント駆動など、
より高速な通信や異なる通信モデルが必要になる場合も存在する。
この領域では、MADOCA/DARUMA や TANGO で用いられてきた
ZMQ / CORBA などのメッセージング技術や、PUB/SUB 型の通信が有利になる。

スサノオでは、Web 系の技術体系との親和性を保ちながら、
高速なバイナリ転送が必要な用途には gRPC を選択できる。
gRPC は Protocol Buffers を用いたバイナリ転送が可能であり、
画像転送など REST が適さない用途を補完する。
この gRPC への拡張は本家 BL774 でも行われており、BL774 対応の一つでもある。

デンソーウェーブの ORiN3 でも gRPC が採用されているように、
機器制御 I/F に一般的な RPC 技術を利用する方向は珍しいものではなくなっている。
スサノオでは REST と gRPC のどちらか一方をシステム全体の標準として固定せず、
対象となるデバイスと必要な通信速度に応じて通信 Frame を選択する。

さらに、分散環境そのものを構成する技術として DDS がある。
DDS を最初からシステムの基盤として採用する設計も一つの選択肢であり、
ROS2 はその代表的な利用例である。
ロボットを扱う以上、DDS / ROS2 との接続はスサノオでも重要になる。

ただし、スサノオは BL774 を基盤としたシステムであるため、
DDS をすべてのデバイスに要求するコア通信基盤とはしない。
DDS が適する領域では DDS / ROS2 と接続し、
高速な RPC が必要なら gRPC、通常の機器制御なら REST / FastAPI を利用する。
TANGO 環境との接続が必要な場合には TANGO Frame を利用する。

つまり、スサノオでは通信 protocol の統一そのものを目的としない。
