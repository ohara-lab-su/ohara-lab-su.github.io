...
SPring-8 の **[BL774 互換](https://user.spring8.or.jp/sp8info/?p=42759)** (ese774)
を用いた一連の計測システム**スサノオ**と読んでいる。
...
フレームを設計し、それを組み合わせる形となる。
システムの中核となる通信フレームは、ターゲットとなるデバイスと目的の通信速度・フレークワークとの
結合をしやすいように、フレームを切り替えても同じように使えるようにする。
基本は**シンプルな透過型プロキシ**を採用している。
透過型であるために、通信は何を採用しても基本は問題がない。
BL774互換としての立ち位置は、RestAPI や gRPC 通信を軸としているところに由来している。
できる限りAPIセットや基本ルールをBL774に近づけたいところだが、現在は、get や post の
ルールを真似ているところくらいかもしれない。
同じ意味で、マネージメントやAPIセットはまだ大きな規定はない。透過型なので特に規程は必要もない。
